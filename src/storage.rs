...
impl AppPaths {
    /// `ZERONE_HOME` 是测试和便携安装的显式覆盖；正常运行使用 `~/.zerone`。
    pub fn discover() -> Result<Self> {
        let root = match std::env::var_os("ZERONE_HOME") {
            Some(path) if !path.is_empty() => PathBuf::from(path),
            _ => user_home_dir()?.join(".zerone"),
        };
...
fn user_home_dir() -> Result<PathBuf> {
    ...
    home.map(PathBuf::from)
        .ok_or_else(|| anyhow!("无法确定用户主目录，请设置 ZERONE_HOME"))
}
